...
<div id="<%=this.ClientID%>" class="em-view-survey-wrap">
    ...
    <script> jQuery(function ($) {
        'use strict';
        $('#<%=this.ClientID%>').engageSurveyView({
            submittingText: '<%=ClientAPI.GetSafeJSString(this.Localize("Submitting"))%>',
            submittingClass: 'submitting',
            validationGroup: '<%=this.SurveyControl.ValidationGroup%>'
        });
    });</script>
</div>