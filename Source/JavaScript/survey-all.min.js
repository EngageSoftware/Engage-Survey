 ...
 */
if(!Array.prototype.indexOf){Array.prototype.indexOf=function(d){if(this===void 0||this===null){throw new TypeError()}var e=Object(this);var b=e.length>>>0;if(b===0){return -1}var c=0;if(arguments.length>0){c=Number(arguments[1]);if(c!==c){c=0}else{if(c!==0&&c!==(1/0)&&c!==-(1/0)){c=(c>0||-1)*Math.floor(Math.abs(c))}}}if(c>=b){return -1}var a=c>=0?c:Math.max(b-Math.abs(c),0);for(;a<b;a++){if(a in e&&e[a]===d){return a}}return -1}}(function(a,b){a(function(){var c=b.Sys.Application.findComponent,e=b.alert,h=b.currentContextInfo,G=a("#Form").validate({ignore:"#DefineAnswerType"}),f="normal",w=[],D=c(a(".ee-start-date .RadPicker input").attr("id")),j=c(a(".ee-end-date .RadPicker input").attr("id"));a.validator.addMethod("email",function(I,H){return this.optional(H)||h.EmailRegex.test(I)});a.validator.addMethod("emails",function(I,H){return this.optional(H)||h.EmailsRegex.test(I)});a.validator.setDefaults({rules:{DefineAnswerType:{min:1},required:{required:true},notificationEmail:{required:{depends:function(){return a("#EvalSendNotification").is(":checked")}},email:true},notificationEmails:{required:{depends:function(){return a("#EvalSendNotification").is(":checked")}},emails:true},thankYouEmail:{required:{depends:function(){return a("#EvalSendThankYou").is(":checked")}},email:true}},messages:h.ErrorMessages,onsubmit:false});b.onbeforeunload=function(){if(a("#CancelQuestion").is(":visible")||a("#EvalCancel").is(":visible")){return h.UnsavedChangedWarning}return undefined};function g(I,J,H){jQuery.ajax({type:"POST",url:h.WebMethodUrl+I+"?portalId="+h.PortalId,data:JSON.stringify(J),contentType:"application/json",success:function(K){if(a.isFunction(H)){H(K.hasOwnProperty("d")?K.d:K)}},error:function(){e(h.ErrorMessage)}})}function l(){return{survey:{SurveyId:a(".ee-create-new").data("surveyId")||-1,Text:a("#EvalTitleInput").val(),StartDate:D.get_selectedDate(),PreStartMessage:a("#EvalPreStartTextArea").val(),EndDate:j.get_selectedDate(),PostEndMessage:a("#EvalPostEndTextArea").val(),SendNotification:a("#EvalSendNotification").is(":checked"),NotificationFromEmailAddress:a("#EvalNotificationFromEmail").val(),NotificationToEmailAddresses:a("#EvalNotificationToEmails").val(),SendThankYou:a("#EvalSendThankYou").is(":checked"),ThankYouFromEmailAddress:a("#EvalThankYouFromEmail").val(),PortalId:h.PortalId,ModuleId:h.ModuleId,RevisingUser:h.UserId,Sections:[{Text:a("#EvalDescTextArea").val()}]}}}function F(H){g("UpdateSurvey",l(),function(I){a(".ee-create-new").data("surveyId",I);if(a.isFunction(H)){H(I)}})}function k(){return{surveyId:a(".ee-create-new").data("surveyId")||-1,question:{QuestionId:a("#CreateQuestions").data("questionId")||-1,Text:a("#QuestionText").val(),IsRequired:a("#QuestionRequiredCheckBox").is(":checked"),RelativeOrder:a("#CreateQuestions").data("relativeOrder")||a(".ee-preview").length+1,ControlType:a("#DefineAnswerType").val(),RevisingUser:h.UserId,Answers:a.map(a("#MultipleAnswer:visible .answer-inputs li.answer-input:visible"),function(I){var H=a(I);return{AnswerId:H.data("answerId")||-1,Text:H.find(":input").val()}})}}}function i(H,L,I,K,J){H.fadeOut("slow",function(){var O,M=H.siblings(".ee-undo").eq(0).clone().show().removeClass("template"),Q=M.html(),R=11,P=new Date(),N;if(a.isFunction(I)){I()}M.html(Q.replace("{0}",'<span class="undo-limit"></span>'));H.addClass("deleted");H.before(M);M.hide().fadeIn(f);if(L){N=function(){M.remove();w.splice(w.indexOf(N),1);if(a.isFunction(K)){K()}};w.push(N);O=setTimeout(N,R*1000);(function S(){var T=new Date(),U=T.getTime()-P.getTime(),V=(R*1000)-U,W=parseInt(V/1000,10);M.find("span.undo-limit").text(W.toString(10));if(W>0){setTimeout(S,300)}}())}M.find("a").click(function(T){T.preventDefault();clearTimeout(O);w.splice(w.indexOf(N),1);M.fadeOut(f,function(){H.removeClass("deleted").fadeIn(f);M.remove()});if(a.isFunction(J)){J()}})})}function m(H){a("#EvalUpdate").parent().fadeOut(f);a("#EvalCancel").parent().fadeOut(f,H)}function E(H,I){H.parent().data("previousValue",I===undefined?H.text():I)}function B(H){H.val(H.parent().data("previousValue"))}function y(H){H.attr("checked",H.parent().data("previousValue"))}function A(H){H.set_selectedDate(a(H.get_element()).closest(".RadPicker").parent().data("previousValue"))}function q(H,I){H.slideUp(f,function(){var K=a(this),L=K.attr("maxlength"),M=K.attr("minlength"),J;if(L<0||L>500000){L=""}if(!M){M=""}J=a("<span />").attr({id:K.attr("id"),className:K.attr("class"),name:K.attr("name")}).data("minlength",M).data("maxlength",L||"").data("rows",K.attr("rows")||"").data("cols",K.attr("cols")||"").data("checked",K.attr("checked")!==undefined?K.is(":checked"):"").addClass("ee-input-pre").text(I||K.val()).hide().insertAfter(K).fadeIn(f);K.remove();J.html(J.html().replace(/\n/g,"\n<br />"))}).addClass("ee-input-pre")}function s(H,I,K,J){K=(K===undefined&&a.isFunction(H.val))?H.val():K;if(K){J=J||q;J(H,K);I.slideDown(f);return true}else{I.slideUp(f,function(){a(this).hide()});return false}}function r(H,I){H.fadeOut(f,function(){var J=a(this),K=J.data("maxlength");I.attr({id:J.attr("id"),className:J.attr("class"),name:J.attr("name"),minlength:J.data("minlength"),rows:J.data("rows"),cols:J.data("cols"),checked:J.data("checked")}).val(J.text()).hide().insertAfter(J).slideDown(f);if(K!==""){I.attr("maxlength",K)}J.remove()}).removeClass("ee-input-pre")}function p(K){var J=a(K.get_element()).closest(".ee-input"),I=J.find(".RadPicker"),H=J.find(".ee-date-pre");if(H.length===0){H=a("<span />").addClass("ee-date-pre").insertAfter(I)}I.slideUp(f,function(){var L=K.get_dateInput();H.text(L.get_dateFormatInfo().FormatDate(K.get_selectedDate(),L.get_displayDateFormat())).hide().fadeIn(f);I.hide()})}function o(K){var J=a(K.get_element()).closest(".ee-input"),I=J.find(".RadPicker"),H=J.find(".ee-date-pre");H.slideUp(f,function(){I.fadeIn(f)})}function u(){var I,H;q(a("#EvalTitleInput"));s(a("#EvalDescTextArea"),a(".ee-description"));H=s(D,a(".ee-start-date"),D.get_selectedDate(),p);I=H;H=s(a("#EvalPreStartTextArea"),a(".ee-pre-start"));I=I||H;H=s(j,a(".ee-end-date"),j.get_selectedDate(),p);I=I||H;H=s(a("#EvalPostEndTextArea"),a(".ee-post-end"));I=I||H;a(".ee-timeframe.ee-expanded legend a").click();if(!I){a(".ee-timeframe").slideUp(f)}s(a("#EvalSendNotification"),a(".ee-notification"),a("#EvalSendNotification").is(":checked")?h.CheckBoxCheckedText:h.CheckBoxUncheckedText);s(a("#EvalNotificationFromEmail"),a(".ee-notification-from"));s(a("#EvalNotificationToEmails"),a(".ee-notification-to"));s(a("#EvalSendThankYou"),a(".ee-thankyou"),a("#EvalSendThankYou").is(":checked")?h.CheckBoxCheckedText:h.CheckBoxUncheckedText);s(a("#EvalThankYouFromEmail"),a(".ee-thankyou-from"));a(".ee-email.ee-expanded legend a").click();a("#EvalEdit").parent().fadeIn(f);a("#EvalDelete").parent().fadeIn(f)}function z(){a("#QuestionText").val("");a("#QuestionRequiredCheckBox").attr("checked",true);a("#DefineAnswerType").find("option:first").attr("selected",true);a("#ShortTextAnswer").slideUp(f);a("#LongTextAnswer").slideUp(f);a("#MultipleAnswer").slideUp(f);a("#CancelQuestion").parent().fadeOut(f);a("#AddNewQuestion").parent().fadeOut(f);a("#MultipleAnswer li.answer-input.deleted").add(".answer-inputs li.ee-undo:not(.template)").remove();a("#MultipleAnswer li.answer-input").remove();var H=a("#MultipleAnswer li.answer-input-template");H.clone(true).attr("class","answer-input").show().insertAfter(H).find(".answer-num").text(2);H.clone(true).attr("class","answer-input").show().insertAfter(H).find(".answer-num").text(1);a(".ai-input input").val("");a("#SaveQuestion").text(h.SaveQuestionButtonText).attr("title",h.SaveQuestionToolTip).parent().addClass("disabled");a("#CreateQuestions").removeData("questionId").removeData("relativeOrder").find("#MultipleAnswer li.answer-input").removeData("answerId");G.resetForm()}function C(N){var K=a("#MultipleAnswer"),M=a("#ShortTextAnswer"),J=a("#LongTextAnswer"),H=a(".ee-define-answer .primary-btn"),L=a("#SaveQuestion").parent(),I=a("#CancelQuestion").parent();L.removeClass("disabled");if(a("#QuestionText").val()||a("#DefineAnswerType :selected").val()>0){I.fadeIn(f)}else{I.fadeOut(f)}switch(N){case 0:J.slideUp(f);M.slideUp(f);K.slideUp(f);H.slideUp(f);L.addClass("disabled");break;case 2:K.slideUp(f);J.slideUp(f);H.fadeOut(f,function(){M.slideDown(f)});break;case 1:M.slideUp(f);K.slideUp(f);H.fadeOut(f,function(){J.slideDown(f)});break;default:J.slideUp(f);M.slideUp(f,function(){K.slideDown(f);H.fadeIn(f)})}}function x(I,L){z();var K=I.data("questionType"),J=I.data("questionId"),H;a("#QuestionText").val(I.children(".pv-question").text());a("#QuestionRequiredCheckBox").attr("checked",I.children(".ee-required-label").text()==="*");if(L){a("#CreateQuestions").data("questionId",J).data("relativeOrder",a("#ee-previews li.ee-preview").index(I)+1)}a("#DefineAnswerType").val(K);C(K);if(K!==2&&K!==1&&K!==0){a("#CancelQuestion").parent().show();H=a(".answer-inputs li.answer-input:last").clone(true);a(".answer-inputs li.answer-input").remove();I.find(".pv-answer").find("input, option").each(function(N){var M=H.clone(true);M.find(".answer-num").text(N+1);M.find("input").val(a(this).text()||a(this).parent().text());M.appendTo(".answer-inputs");if(L){M.data("answerId",a(this).data("answerId"))}})}}function d(M,O,L,P,K){var N=a("#CreateQuestions").data("relativeOrder"),J,H,I;if(N){J=a(".ee-preview").eq(N-1)}else{J=a(".ee-preview-template").clone(true).attr("class","ee-preview");a("#ee-previews").append(J)}J.find(".pv-question").text(O).show();J.find(".ee-required-label").text(L?"*":"").show();J.show().data("questionId",M).data("questionType",P);H=J.find(".pv-answer").empty();switch(P){case 2:H.html("<input type='text' class='NormalTextBox' />");break;case 1:H.html("<textarea class='NormalTextBox' />");break;case 5:I=a("<select class='NormalTextBox dropdown-prev'></select>");H.append(I);a.each(K,function(R,Q){a("<option>"+Q.Text+"</option>").appendTo(I).data("answerId",Q.AnswerId)});break;case 3:a.each(K,function(R,Q){a("<label><input type='radio' name='"+M+"' />"+Q.Text+"</label>").appendTo(H).find("input").data("answerId",Q.AnswerId)});break;case 6:a.each(K,function(R,Q){a("<label><input type='checkbox' />"+Q.Text+"</label>").appendTo(H).find("input").data("answerId",Q.AnswerId)});break;default:e("todo: implement validation, shouldn't be able to add a question if you have 'select answer type' selected in the drop down.")}}a(".ee-collapsed legend a, .ee-expanded legend a").click(function(J){J.preventDefault();var I=a(this).closest(".ee-collapsed, .ee-expanded"),H=I.find(".ee-collapsable");H.slideToggle(f,function(){I.toggleClass("ee-collapsed").toggleClass("ee-expanded")})});a("#ee-previews").sortable({items:"li.ee-preview",placeholder:"ui-state-highlight"});a(".answer-inputs").sortable({items:"li.answer-input",placeholder:"ui-state-highlight"});a("#ee-previews").bind("sortupdate",function(){var I={},H;a("#ee-previews li.ee-preview:visible").each(function(K,J){I[a(J).data("questionId")]=K+1});H={surveyId:a(".ee-create-new").data("surveyId"),questionOrderMap:I};g("ReorderQuestions",H)});a(".answer-inputs").bind("sortupdate",function(){var H=a(".answer-inputs li.answer-input:visible").find(".answer-num");H.each(function(J,I){a(I).text(J+1)})});a("#engage-evaluation :input").focus(function(){a(this).addClass("focus")}).blur(function(){a(this).removeClass("focus")});a("#EvalNew").click(function(I){I.preventDefault();if(G.form()){var H=a(this),J=H.text();H.text(h.ProgressText);F(function(){a("#EvalNew").parent().fadeOut(f,function(){u()});a(".ee-create-questions").show();H.text(J)})}});a("#EvalUpdate").click(function(I){I.preventDefault();if(G.form()){var H=a(this),J=H.text();H.text(h.ProgressText);F(function(){m(function(){u()});H.text(J)})}});a("#EvalEdit").click(function(H){H.preventDefault();E(a("#EvalTitleInput"));E(a("#EvalDescTextArea"));E(a(".ee-start-date .RadPicker"),D.get_selectedDate());E(a("#EvalPreStartTextArea"));E(a(".ee-end-date .RadPicker"),j.get_selectedDate());E(a("#EvalPostEndTextArea"));E(a("#EvalSendNotification"),a("#EvalSendNotification").is(":checked"));E(a("#EvalNotificationFromEmail"));E(a("#EvalNotificationToEmails"));E(a("#EvalSendThankYou"),a("#EvalSendThankYou").is(":checked"));E(a("#EvalThankYouFromEmail"));r(a("#EvalTitleInput"),a('<input type="text"/>'));r(a("#EvalDescTextArea"),a("<textarea/>"));o(D);r(a("#EvalPreStartTextArea"),a("<textarea/>"));o(j);r(a("#EvalPostEndTextArea"),a("<textarea/>"));r(a("#EvalSendNotification"),a('<input type="checkbox"/>'));r(a("#EvalNotificationFromEmail"),a('<input type="text"/>'));r(a("#EvalNotificationToEmails"),a('<input type="text"/>'));r(a("#EvalSendThankYou"),a('<input type="checkbox"/>'));r(a("#EvalThankYouFromEmail"),a('<input type="text"/>'));a(".ee-create-new .ee-optional").show();a("#EvalEdit").parent().fadeOut(f,function(){a("#EvalCancel").parent().fadeIn(f);a("#EvalUpdate").parent().fadeIn(f)});G=a("#Form").validate()});a("#EvalCancel").click(function(H){H.preventDefault();B(a("#EvalTitleInput"));B(a("#EvalDescTextArea"));A(D);B(a("#EvalPreStartTextArea"));A(j);B(a("#EvalPostEndTextArea"));y(a("#EvalSendNotification"));B(a("#EvalNotificationFromEmail"));B(a("#EvalNotificationToEmails"));y(a("#EvalSendThankYou"));B(a("#EvalThankYouFromEmail"));m(function(){u()});G.resetForm()});a("#EvalDelete").click(function(I){I.preventDefault();i(a("#engage-evaluation"),true,null,function H(){g("DeleteSurvey",{surveyId:a(".ee-create-new").data("surveyId")},function(){b.location=a(".egn-home a").attr("href")})})});a("#AddNewQuestion").click(function(K){K.preventDefault();var I=a(".answer-input:visible:last .answer-num"),H=a(".answer-input-template").clone(true).attr("class","answer-input").hide().appendTo(".answer-inputs").slideDown(f),J=parseInt(I.text(),10);H.find(".answer-num").text(J+1);H.find("input").val("").focus();a(".answer-inputs .ee-delete").removeClass("disabled")});a(".answer-inputs .ee-delete").click(function(L){L.preventDefault();var H=a(".answer-inputs li.answer-input:visible"),I;if(H.length>1){I=a(this).closest("li");i(I,false,function J(){H=a(".answer-inputs li.answer-input:visible").each(function(N,M){a(M).find(".answer-num").text(N+1)});if(H.length<2){H.find(".ee-delete").addClass("disabled")}},null,function K(){a(".answer-inputs li.answer-input:visible").each(function(N,M){a(M).find(".answer-num").text(N+1)}).find(".ee-delete").removeClass("disabled")})}});a(".ee-pr-action-links .ee-edit").click(function(I){I.preventDefault();var H=a(this).closest("li.ee-preview");x(H,true);a("#SaveQuestion").text(h.UpdateQuestionButtonText).attr("title",h.UpdateQuestionToolTip)});a(".ee-pr-action-links .ee-copy").click(function(I){I.preventDefault();var H=a(this).closest("li.ee-preview");x(H,false);a("#SaveQuestion").text(h.SaveQuestionButtonText).attr("title",h.SaveQuestionToolTip)});a(".ee-pr-action-links .ee-delete").click(function(J){J.preventDefault();var H=a(this).closest("li.ee-preview");i(H,true,null,function I(){var K=H.data("questionId");g("DeleteQuestion",{questionId:K},function(){H.remove()})})});a(b).unload(function(){a.each(w,function(I,H){if(a.isFunction(H)){H()}})});a("#DefineAnswerType").change(function(){C(parseInt(a(this).val(),10))});a("#QuestionText").blur(function(){C(parseInt(a("#DefineAnswerType").val(),10))});a("#SaveQuestion").click(function(H){H.preventDefault();var J=a("#DefineAnswerType :selected").val(),I=J>2;G=a("#Form").validate();if(a("#QuestionText").valid()&&(!I||a(".ai-input input:visible").valid())&&(a("#DefineAnswerType").valid())){a(this).text(h.ProgressText).parent().addClass("disabled");g("UpdateQuestion",k(),function(K){a("#PreviewArea").slideDown(f);d(K.QuestionId,a("#QuestionText").val(),a("#QuestionRequiredCheckBox").is(":checked"),parseInt(a("#DefineAnswerType :selected").val(),10),K.Answers);z()})}});a("#CancelQuestion").click(function(H){H.preventDefault();z()});function v(H){if(H){return new Date(parseInt(H.replace("/Date(","").replace(")/",""),10))}return null}(function n(){if(D===null||j===null){setTimeout(function(){D=c(a(".ee-start-date .RadPicker input").attr("id"));j=c(a(".ee-end-date .RadPicker input").attr("id"));n()},1);return}if(h.Survey){a(".ee-create-new").data("surveyId",h.Survey.SurveyId);a("#EvalTitleInput").val(h.Survey.Text);a("#EvalDescTextArea").val(h.Survey.Sections[0].Text);D.set_selectedDate(v(h.Survey.StartDate));a("#EvalPreStartTextArea").val(h.Survey.PreStartMessage);j.set_selectedDate(v(h.Survey.EndDate));a("#EvalPostEndTextArea").val(h.Survey.PostEndMessage);a("#EvalSendNotification").attr("checked",h.Survey.SendNotification);a("#EvalNotificationFromEmail").val(h.Survey.NotificationFromEmailAddress);a("#EvalNotificationToEmails").val(h.Survey.NotificationToEmailAddresses);a("#EvalSendThankYou").attr("checked",h.Survey.SendThankYou);a("#EvalThankYouFromEmail").val(h.Survey.ThankYouFromEmailAddress);a("#EvalNew").parent().hide();u();m();a(".ee-create-questions").show();if(h.Survey.Sections[0].Questions.length){a("#PreviewArea").show();a.each(h.Survey.Sections[0].Questions,function(H,I){d(I.QuestionId,I.Text,I.IsRequired,I.ControlType,I.Answers)})}}else{a("#EvalSendNotification").attr("checked",h.DefaultEmailSettings.SendNotification);a("#EvalNotificationFromEmail").val(h.DefaultEmailSettings.NotificationFromEmail);a("#EvalNotificationToEmails").val(h.DefaultEmailSettings.NotificationToEmails);a("#EvalSendThankYou").attr("checked",h.DefaultEmailSettings.SendThankYou);a("#EvalThankYouFromEmail").val(h.DefaultEmailSettings.ThankYouFromEmail)}z()}())})}(jQuery,this));